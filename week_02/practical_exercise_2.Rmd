...
- Scenario. The subjects are presented to different scenarios.
- Attitude. Whether it is the formal or informal condition of the scenario.
- Total duration. Amount of time the subject uses to "complete" the scenario. 
- f0mn. Base frequency. Pitch (in Hz).
...
Scenario is treated as something where we have a limited set of possible values (specifically the seven scenarios we see in the data set, I think). So a scenario either is one of seven or not, and R gives us a TRUE/FALSE (where 1 is true and 0 is false) answer to all of the seven possibilities.
...
Well, first of all the males have a way lower f0mn, which is very much expected. Other than that, a few of the participants vary way more in f0mn according to the scenario than others. It looks like the informal pitch is a liiiiittle bit higher than the formal one. 
...
model2 <- lmer(f0mn~gender + (1|scenario), politeness)
model3 <- lmer(f0mn~gender + (1|subject), politeness)
model4 <- lmer(f0mn~gender + (1|scenario) + (1|subject), politeness)
```
