...
m <- glmer(correct ~ target.frames + (1|subject), family = binomial, data = df)
m_ranslope <- glmer(correct ~ target.frames(target.frames|subject), family = binomial, data = df) #is this what the text means?
m_target <- glmer(correct ~ 1 + target.frames + (target.frames|subject), family = binomial, data = df)
m_target_no_cor <- glmer(correct ~ 1 + target.frames + (target.frames||subject), family = binomial, data = df) #isn't this identical to the one above?? 
...
df %>% 
  ggplot() +
   geom_smooth(aes(x = target.frames, y = fitted(m_target), color = "Partial Pooled")) + 
