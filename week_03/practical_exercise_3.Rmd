...
df_staircase %>% 
  ggplot(aes(x = target.contrast, y = fitted_nopooling))+
  geom_point()+
...
geom_point(aes(x = target.contrast, y = fitted(logistic_model_pooling), color = "complete pooling")) +
...
tibble(sum(residuals(correctmodel)^2), sum(residuals(correctmodel2)^2), sum(residuals(correctmodel3)^2), sum(residuals(correctmodel4)^2))
```
What a good model 
